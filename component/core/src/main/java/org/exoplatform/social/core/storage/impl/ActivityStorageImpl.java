...
//      }
  ...
  private List<ExoSocialActivity> getActivityOfInactiveUser(Identity ownerIdentity, int offset, int limit) {
    ...
    } else if (includedActiveGroups(ownerIdentity.getRemoteId())) {
      //user is high priority
      return new LinkedList<ExoSocialActivity>();
    } else {
      //"inactive" user
      return getNewerActivitiesForInactiveUser(ownerIdentity, offset, limit, beforeLastLogin);
    }
  }
  ...
  private List<ExoSocialActivity> getNewerActivitiesForInactiveUser(Identity ownerIdentity,
    ...
    try {
      int days = StreamContext.instanceInContainer().getLastLoginAroundDays();
   ...
   * @return TRUE/FALSE
   */
  private boolean includedActiveGroups(String userId) {
    String userGroups = StreamContext.instanceInContainer().getActiveUserGroups();
